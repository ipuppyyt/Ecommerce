  ...
  return (
    <>
      ...
      <div className="max-w-7xl w-full mx-auto my-24 shadow-md">
          ...
          <div className="flex flex-wrap justify-around py-2.5" key={products.id}>
            <div className="max-w-lg min-w-[290px] overflow-hidden m-6.25">
              <img src={products.src[index]} alt="" className="w-full h-full max-h-100 object-cover" />
            </div>

            <div className="max-w-lg min-w-[290px] m-6.25">
              <div className="flex justify-between mb-3.75">
                <h2 className="uppercase tracking-wide">{products.name}</h2>
                <span className="text-crimson">${products.price}</span>
              </div>
              <p className="leading-6 my-3.75">{products.description}</p>
              

              <div className="w-20 h-25 flex cursor-pointer my-2.5">
                {products.src.map((image:any, idx:any) => (
                  <img
                    key={idx}
                    src={image}
                    alt=""
